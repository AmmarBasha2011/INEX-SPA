...
runTest('Firewall - no config file', function() {
    ...
    if (file_exists($configPath)) rename($configPath, $configPath . '.bak');
...
foreach ($blockedIPs as $i => $ip) {
    runTest("Firewall IP Block Logic Test $i ($ip)", function() use ($ip) {
...
foreach ($blockedUAs as $i => $ua) {
    runTest("Firewall User-Agent Block Logic Test $i ($ua)", function() use ($ua) {
        assertTrue(true);
    });
}
